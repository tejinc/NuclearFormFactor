...
vector<double> AEp({1, -0.0578, 0, 11.1, 13.6,33.0,0});
vector<double> AMp({1, 0.150, 0, 11.1, 19.6, 7.54, 0});
vector<double> AEn({0, 1.25, 1.30, -9.86, 305, -758, 802});
vector<double> AMn({1, 1.81, 0, 14.1, 20.7, 68.7, 0});
...
double G( vector<double> &par, double tau )
{
  //double num = 0;
  //double denum = 1;
  //for( int k = 0; k< 3; k++ ) num+=par[k]*pow(tau,k);
  //for( int i = 3; i< 6; i++ )
  //{
  //  int k = i-2;
  //  denum+=par[i]*pow(tau,k);
  //} 

  double num = par[0]+par[1]*tau+par[2]*tau*tau;
  double denum = 1 + par[3]*tau+par[4]*tau*tau+par[5]*tau*tau*tau+par[6]*tau*tau*tau*tau;
  return num/denum;
...
double G(string name, double Q2)
{
  if(name == "Mp") 
  {
    return G( AMp, tau(Q2, true) );
  }
  if(name == "Ep") 
  {
    return G( AEp, tau(Q2, true) );
  }

  if(name == "Mn") 
  {
    return G( AMn, tau(Q2, false) );
  }

  if(name == "En") 
  {
    return G( AEn, tau(Q2, false) );
  }
  return -99999;
}
double GMp(double Q2) { return G("Mp",Q2)*up; }
double GMn(double Q2) { return G("Mn",Q2)*un; }
...
double GEp2(double Q2)
{
  ...
  return GD(Q2,MV,false );
}
double GMp2(double Q2)
{
  double MV = TMath::Sqrt(0.71); 
  return GD(Q2,MV,false )*up;
}
